...
o.spec('PatchFinder', () => {
    ...
    o('should find the function', () => {
        const search = makeProto();
        const base = { search };
        const res = ClientLibPatch.findFunctionInProto(base, 'search.ret', 'return 1');
        o(res).equals(search.ret.prototype.foo.toString());
    });

    o('should extract a value from function', () => {
        const search = makeProto();
        const base = { search };
        const res = ClientLibPatch.extractValueFromFunction(base, 'search.ret', 'return 1', /([0-9])/);
        o(res).equals('1');
    });
    ...
    o('should extract base colors from $ctor', () => {
        const search = makeProto();
        const base = { search };
        const res = ClientLibPatch.extractValueFromFunction(
            base,
            'search.ret',
            '.Color=',
            /.*\.([A-Z]{6})\(this.*Color=.*/,
